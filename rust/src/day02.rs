...
fn parse_color(input: &str) -> IResult<&str, String> {
    map(take_while(|c: char| c.is_alphabetic()), String::from)(input)
}
...
fn parse_round(input: &str) -> IResult<&str, Round> {
    ...
    let (input, color_counts) = separated_list0(tag(", "), parse_color_count)(input)?;

    for color_count in color_counts {
        let ColorCount { color, count } = color_count;
        map.insert(color, count);
    }
    Ok((input, Round { color_counts: map }))
}

fn parse_game(input: &str) -> IResult<&str, Game> {
    let (input, _) = space0(input)?; // Skip leading spaces if any
    let (input, game_number) = preceded(tag("Game "), parse_u32)(input)?;
    let (input, rounds) =
        delimited(tag(": "), separated_list0(tag("; "), parse_round), space0)(input)?;
...
pub fn part1(input: &str) -> usize {
    let (_, games) = parse_games(input.trim()).unwrap();

    games
        .into_iter()
        .filter(is_possible)
        .map(|g| g.game_number as usize)
        .sum()
}
...
fn is_possible(game: &Game) -> bool {
    let blue = game
        .rounds
        .iter()
        .map(|x| x.color_counts.get("blue").map(|&v| v).unwrap_or(0))
        .max()
        .unwrap_or(0);
    let red = game
        .rounds
        .iter()
        .map(|x| x.color_counts.get("red").map(|&v| v).unwrap_or(0))
        .max()
        .unwrap_or(0);
    let green = game
        .rounds
        .iter()
        .map(|x| x.color_counts.get("green").map(|&v| v).unwrap_or(0))
        .max()
        .unwrap_or(0);
...
pub fn part2(input: &str) -> u32 {
    ...
    let x = games.into_iter().map(power);
    x.sum()
}
...
fn power(game: Game) -> u32 {
    let blue = game
        .rounds
        .iter()
        .map(|x| x.color_counts.get("blue").map(|&v| v))
        .filter(|&p| p.is_some())
        .map(|p| p.unwrap_or(0))
        .max()
        .unwrap_or(0);
    let red = game
        .rounds
        .iter()
        .map(|x| x.color_counts.get("red").map(|&v| v))
        .filter(|&p| p.is_some())
        .map(|p| p.unwrap_or(0))
        .max()
        .unwrap_or(0);
    let green = game
        .rounds
        .iter()
        .map(|x| x.color_counts.get("green").map(|&v| v))
        .filter(|&p| p.is_some())
        .map(|p| p.unwrap_or(0))
        .max()
        .unwrap_or(0);
...
Game 100: 1 red, 5 blue, 2 green; 3 red, 1 blue; 1 green, 1 blue, 1 red"#;
    ...
    fn part1_example_test() {
        let result = part1(INPUT_EXAMPLE);
        assert_eq!(result, 8);
    }
    ...
    fn part2_example_test() {
        let result = part2(INPUT_EXAMPLE);
